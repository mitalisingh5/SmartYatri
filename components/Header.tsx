import React from 'react';
import type { View } from '../types';

interface HeaderProps {
    onLogout: () => void;
    setCurrentView: (view: View) => void;
}

const Header: React.FC<HeaderProps> = ({ onLogout, setCurrentView }) => (
  <header className="bg-white/80 backdrop-blur-sm sticky top-0 z-10 border-b border-slate-200">
    <nav className="container mx-auto px-4 lg:px-8 flex justify-between items-center py-4">
      <button onClick={() => setCurrentView('form')} className="flex items-center space-x-2">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 text-orange-500">
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418" />
        </svg>
        <span className="text-xl font-bold text-slate-800">Smart Trip Planner</span>
      </button>
      <div className="flex items-center space-x-6 text-sm font-medium text-slate-600">
        <button onClick={() => setCurrentView('form')} className="hidden md:inline-block hover:text-orange-500 transition-colors">Plan Trip</button>
        <button onClick={() => setCurrentView('my_itineraries')} className="hidden md:inline-block hover:text-orange-500 transition-colors">My Itinerary</button>
         <button 
          onClick={onLogout}
          className="bg-orange-500 text-white px-4 py-2 rounded-md hover:bg-orange-600 transition-colors font-semibold"
        >
          Logout
        </button>
      </div>
    </nav>
  </header>
);

export default Header;
